...
func _ready() -> void:
	...
	if Global.minigames_done < 3: # if you havent completed 3 minigames yet 
		...
		get_tree().change_scene_to_file("res://scenes/minigame_" + str(Global.minigames_done) + ".tscn") # changes your 
		# scene by arranging this frankenstein path. 
...
#lives visually so later you can always find alternative code. Now, dw abt it.
	...
	level.text = "Level " + str(Global.minigames_done) # this tells you what minigame you're on using concatenation (google the word)
